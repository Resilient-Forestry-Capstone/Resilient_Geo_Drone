...
class GapDetector:
    ...
    """
    def __init__(self, config_loader : ConfigLoader, auto_process: bool = True):
        self.config_loader = config_loader
        self.config = config_loader.load()
        self.gap_config = self.config['geospatial']['gap_detection']
        self.logger = LoggerSetup().get_logger()
        # Use Non-Interactive Mode If We Are On Worker Thread (matplotlib Has Issues With GUI In Worker)
        if auto_process:
            mpl_use('Agg')  # Use 'Agg' backend for non-interactive plotting  
...
if __name__ == "__main__":
    ...
    gap_detector = GapDetector(config_loader, False)
    gap_areas = gap_detector.process_gaps(r"C:\Users\bensp\OneDrive\Desktop\Code Box Two\Python\GeoDrone\ResilientRepo\Resilient_Geo_Drone\data\output\point_cloud\20250410_174353\four_seventy_chm.tif")